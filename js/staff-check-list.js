...
function dataShow(num, category, date) {
  ...
  $.ajax({
    type: 'GET',
    url: 'api/search.php',
    data: { type: 'show_data', category: category, page: num, date: date },
    ...
    success: (result) => {
      ...
      if (result.status === 'false') {
        ...
        if (result.punch_list !== null) {
          result.punch_list.forEach(function (item, value) {
            $('.rev-form-tab tbody').append(`
              <tr class="rev-form-border">
                <td>
                  <div class="dfxcenter">${item["date"]}</div>
                </td>
                <td>${item["name"]}</td>
                <td>
                  <div class="dfxcenter">${item["work"]}</div>
                </td>
                <td>
                  <div class="dfxcenter">${item["back"]}</div>
                </td>
                <td>
                  <div class="dfxcenter">${item["calendar"]}</div>
                </td>
              </tr>`);
          });
        } else {
          $('.rev-form-tab tbody').html("<span>尚無員工打卡紀錄</span>");
        }
      }
    }
  });
...
function pageCount(category, date) {
  $.ajax({
    ...
    data: { type: 'count', category: category, date: date },
    contentType: "application/json; charset=utf-8",
    success: (result) => {
      $('.pagination').html('');
